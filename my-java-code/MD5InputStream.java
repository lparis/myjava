...
public class MD5InputStream extends FilterInputStream {
  ...
  public static void main(String[] arg) {
	...
	System.out.println(FastMD5.asHex(digest.digest())+"  "+filename);
	in.close();
	...
	if (!use_native_lib) {
	  FastMD5.initNativeLibrary(true);
	}
	...
	while ((num_read = in.read(buf)) != -1);
	System.out.println(FastMD5.asHex(in.hash())+"  "+filename);
	in.close();
      }
    } catch (Exception e) {
      e.printStackTrace();
